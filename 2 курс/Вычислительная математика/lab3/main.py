...
def finalPrompt(res, n, eps):
    if res is not None:
        print("\n" + "=" * 60)
        print("Результат:")
        print("=" * 60)
        print(f"Интеграл ≈ {res:.3f}")
...
def firstTask():
    ...
    res, nFinal = solveRunge(f, a, b, methodFunc, k, eps)

    finalPrompt(res, nFinal, eps)
...
def secondTask():
    ...
    if breakType == "1":
        res, nFinal = solveRunge(f_imp, a, b, methodFunc, k, eps, alpha=eps)
    elif breakType == "2":
        res, nFinal = solveRunge(f_imp, a, b, methodFunc, k, eps, delta=eps)
    else:
        ...
        if n_left is None or n_right is None:
            ...
            print("\n\n")
            return
        res, nFinal = I_2n_left + I_2n_right, n_left + n_right
