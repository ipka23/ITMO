...
print_prompt:
    ...
    sb    t3, 0(t0)              ; вывод tmp в io[0x84]
...
check_name_length:
    bgt   a4, a7, overflow       ; if n + m > buffer_max_length

    lb    t3, 0(t6)
    beq   t4, t3, restore_buf2_addr  ; if tmp == '\n'

    sb    t3, 0(sp)              ; запись в mem[buf2]
    addi  sp, sp, 1              ; buf2++
    addi  a4, a4, 1
    j     check_name_length
...
print_hello:
    ...
    lb    t3, 0(t1)
    sb    t3, 0(a0)              ; запись в mem[0..31]
    sb    t3, 0(t0)              ; вывод в io[0x84]

    addi  t2, t2, -1             ; n--
    addi  t1, t1, 1              ; ptr++
    addi  a0, a0, 1              ; buffer_ptr++
...
print_name:
    mv    a1, a4
    addi  a1, a1, -7             ; a1 = m (n + m - n)

loop_print_name:
    beqz  a1, end_print_name     ; if m == 0
...
end_print_name:
    sb    t5, 0(t0)              ; '!' в io[0x84]
    sb    t5, 0(a0)              ; запись '!' в mem[0..31]
    addi  a4, a4, 1
    addi  a0, a0, 1
...
loop:
    beqz  a3, end                ; if buffer_length == 0
    sb    a2, 0(a0)              ; если осталось место в mem[0..31], заполняем его '_'
    addi  a0, a0, 1
    addi  a3, a3, -1
    j     loop
