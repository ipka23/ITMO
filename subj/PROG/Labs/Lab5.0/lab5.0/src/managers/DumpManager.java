...
public class DumpManager {
     ...
     * @return CSV-строка
     */
    private String collection2CSV(Collection<Aboba> collection) {
        try {
            StringWriter sw = new StringWriter();
            CSVWriter csvWriter = new CSVWriter(sw, ';');
            for (var e : collection) {
                csvWriter.writeNext(Aboba.toArray(e));
            }
     ...
     */
    public void writeCollection(Collection<Aboba> collection) {
     ...
     * @return коллекция
     */
    private LinkedList<Aboba> CSV2collection(String s) {
        try {
            StringReader sr = new StringReader(s);
            CSVReader csvReader = new CSVReader(sr, ';');
            LinkedList<Aboba> ds = new LinkedList<Aboba>();
            String[] record = null;
            while ((record = csvReader.readNext()) != null) {
                Aboba d = Aboba.fromArray(record);
                if (d.validate())
                    ds.add(d);
                else
    ...
    /**
     * Считывает коллекцию из файлa.
     * @return Считанная коллекция
     */
    public void readCollection(Collection<Aboba> collection) {
        ...
        collection = new LinkedList<Aboba>();
    }
}